...
[0.8.1](#升级到-081) · [0.8.0](#升级到-080) · [0.7.0](#升级到-070) · [0.6.0](#升级到-060) · [0.5.1](#升级到-051) · [0.5.0](#升级到-050) · [0.4.0](#升级到-040) · [0.3.0](#升级到-030) · [0.2.x](#升级到-02x) · [0.1.3](#升级到-013) · [0.1.1 / 0.1.2](#升级到-011--012)

## 升级到 0.8.1

0.8.0 → 0.8.1 只有一个出口面修复，**没有任何需要改代码的地方**——既有代码照常编译运行。

- **`compareSnapshots` 的具名导入现在可用**：`docs/GUIDE.md` 教的是 `import { createSnapshot, createSnapshotAsync, compareSnapshots } from '@openlide/geomstore/extras/snapshot'`，而这个名字在 **0.7.0 与 0.8.0 上都不在任何子路径的出口面**（实际导出只有 `SnapshotManager / createSnapshot / createSnapshotAsync / default`），照文档写会拿到 `TypeError: compareSnapshots is not a function`。0.8.1 起 `extras/snapshot` 与聚合入口 `extras` 都导出了它。
  - 若你此前撞到过这个报错、当初改用了类方法（`new SnapshotManager().compareSnapshots(a, b)`，一直可用）或深路径导入，现在可以直接换回具名导入——两条路径是同一个纯函数，签名逐字相同。
  - 类方法**没有**被移除或改动，既有调用无需修改。

## 升级到 0.8.0

0.7.x → 0.8.0 是一轮全库复审（`@alibaba-group/open-code-review` delegate 模式，两轮：第一轮全库找问题、第二轮只评审修复本身）的产出，对外语义变化以 [CHANGELOG](../CHANGELOG.md) 的 0.8.0 一节为准，本节只展开**需要改代码的 1 条**与**无需改代码但断言 / 监控要复核的 3 条**，其余属「原本就该如此」的缺陷修正。**无破坏性类型面变更**——本版只增类型导出，不删、不收紧，既有代码不需要为升级改任何 import。

> **同样不会自动发生**：本版含行为变更，按 0.x 的语义升的是 **minor**（0.7.0 → 0.8.0）。`^0.7.0` 展开为 `>=0.7.0 <0.8.0`，要拿到这一轮修复请把依赖显式改成 `^0.8.0`。

### 需要改代码

- **慢于 10s 的上报端要显式调大 `HttpReporter` 的 `timeout`**：fetch 路径此前**没有任何请求级超时**——`options.timeout` 被展开进 `RequestInit` 后被 fetch 静默忽略，服务端接受连接却不响应时请求永不结束。现在 `timeout` 翻译成 `AbortController` 中止且**缺省 10s**，超时会真的中断这次上报。若你的错误上报端点本来就慢（跨区、大包、自建网关），传 `timeout: 30000` 之类的值；要完全关掉请求级超时传 `timeout: 0`。注意 `NaN` / `Infinity` 会被归一到默认 10s，不要拿它们表达「不超时」。**`wx.request` 路径不受影响**（走平台原生 `timeout`，本库不注入默认值），要同口径需显式传。附带修好的问题：`ErrorMonitoring` 的 `reportTimeout` 判超时后会把批次重入队，而此前底层请求仍在飞、迟到落地即造成同一批错误重复投递——现在 fetch 路径到点真的会结束。

### 行为变更（无需改代码，但断言 / 监控需复核）

- **按 `compareSnapshots` 的 `changed` 做回滚 / 去重的消费方要复核**（两处口径收紧）：① **数组的非索引自有键**（`arr.version = 2`）现在参与比较，差异路径是 `root.list.version`、增删同样报 `kind`；此前只按下标比较，所以 `list.version` 从 1 改成 2 会得到 `changed: false`。② **住在内部槽位的值改为按引用比较**（`ArrayBuffer` / TypedArray / `DataView` / `Promise` / `WeakMap` / `WeakSet` / `Error`）——它们自有可枚举键恒为空，此前任意两个实例恒判无差异；现在换一个实例即记一次差异。净效果是 `changed` **更可能为 `true`**（此前是漏报）。判读顺序不变：先判 `inputTrusted`，再读 `changes`。
- **`ErrorRecovery` 的 `maxRetries` 现在会被归一**：小数向下取整、负数夹到 `0`、非有限值（`NaN` / `Infinity`）回落到默认 3。传了 `NaN` 的配置此前会让重试上限彻底失效（`currentAttempt >= NaN` 恒为假），现在是 3。`0` 仍是「首次失败即放弃」，未变。要表达「不限制」没有受支持的写法——请改用一个足够大的有限值。
- **`deepEqual` 对装箱 `-0` 的口径变了**：`deepEqual(new Number(-0), new Number(0))` 从 `false` 变成 `true`，与 `deepEqual(-0, 0)` 一致（此前同一个值只因有没有装箱就得出相反结论）。用 `deepEqual` 当选择器缓存命中判定的地方，命中集合因此略有变化——若你的选择器刻意区分这两种值，请改用 `Object.is` 自己判。
