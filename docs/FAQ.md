...
**RETRY 的额度语义（重要）：** 重试额度按故障周期计量，周期以时间窗判定（窗口 = `max(60s, 本周期全部退避总时长 × 2)`）。像上面这样「每次失败 `createError` 出新实例再 `recover`」是受支持的常见用法——窗口内换新实例**不会**重置额度，连续失败仍会在 `maxRetries` 次后达到上限（抛 `Max retries exceeded`）；超过窗口未再出现同类错误才视为新周期、额度重置。达到上限时只清除当前错误键（`code:storeName:operation`），同码其他 store/操作的重试额度不受影响。
