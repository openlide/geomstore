...
import { SnapshotManager, createSnapshot, createSnapshotAsync } from '../../../../src/extras/snapshot/SnapshotManager.js'
...
describe('SnapshotManager', () => {
  ...
  describe('同步快照异常处理', () => {
    ...
    test('应该在抛出非 Error 对象时正确处理', () => {
      ...
      // 注：自定义克隆器的抛错已收敛到 invokeCustomCloner（同步/异步同语义），
      // onError 拒绝时统一抛 SnapshotAbortError，其 message 取兜底文案
      const result = manager.createSnapshot(data, {
        customCloner: () => {
          throw 'string error'
        },
...
describe('覆盖率补全：克隆降级与中止路径', () => {
  ...
  test('COV-R5-004: 异步 processQueue 遇未预期内部异常时落账跳过，且不得兜底填入活引用', async () => {
    ...
    // processNodeAsync 已拆至 clone-async.js（模块级纯函数），故改为对模块导出打桩；
