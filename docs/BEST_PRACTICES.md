...
- **不要依赖 `this` 的类型推导去偷懒**：action 上下文的 `state` 为只读引用，写入请走 `setState` / `$patch`。
...
- **重计算用选择器**：`createSelector` 的命中判定优先走状态版本号（O(1)），大状态树下比逐次 `deepEqual` 快几个数量级。
...
- **`onlyOnChange` 的适用场景**：状态面积大、写入频繁但多数不改变内容（如轮询返回相同快照）。它引入脏跟踪代理，状态很小且每次必然变化时收益为负。
...
- **生命周期**：组件端只认 `lifetimes` 写法；已销毁的 Store 上的写操作会抛错，`onUnload` / `detached` 之后不要再触发写入。
