...
> **为什么这个 patch 版本里有破坏性变更**：本库还在 0.x，仓库沿用了「复审修复都进 0.5.x」的节奏，但下面「需要改代码」一节的类型收紧与判据反转按 semver 应升 minor。**`^0.5.1` 展开为 `>=0.5.1 <0.6.0`，也就是说这一版会自动装进你的依赖**——不想被动升级的宿主请把版本钉成 `0.5.1` 或 `=0.5.1`，再按本节逐条改。
