...
- **两份产物、两条链路**：`dist/` 是给 Node 与打包器的 ESM（`exports` 指向它）；`dist-weapp/` 是给微信「构建 npm」的 **按模块一比一转译的 CJS**（105 个模块与 `dist` 一一对应、模块间保留相对 `require`），由包根 `miniprogram` 字段指向、工具整目录拷贝。**不要改成「bundle 成单文件」**：esbuild 的 `--splitting` 只支持 esm，CJS 多入口 bundle 会把 core 重复内联（实测 457.1 KB vs 228.3 KB），更要命的是每个入口各持一份模块实例，`.` 与 `./core` 的 `globalRegistry` 会变成两个对象——`verify:weapp` 里的「跨入口单例同一性」断言就是钉这一条的。改 `exports` 时两条链路都要顾：`build-weapp.mjs` 的入口清单是从 `exports` 派生的（`scripts/weapp-entries.mjs` 单一实现），子路径的 `default` 不是 `./dist/**.js` 形状时构建直接失败，而不是悄悄少发一个入口。**动这块前必读 [docs/WECHAT_NPM_FIX.md](./docs/WECHAT_NPM_FIX.md)**：0.6.0 的坏产物能发出去，就是因为 `test` / `build` / `pack` 没有一个会加载要被微信拷走的那份文件
